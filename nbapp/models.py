...
class UserInfo(AbstractUser):
    ...
    dep=models.ForeignKey('Department',null=True,on_delete=True)
    def __str__(self):
...
class Menu(models.Model):
    title=models.CharField(verbose_name="一级菜单",max_length=32,null=True,blank=True)
...
class Permission(models.Model):
    ...
    pid=models.ForeignKey('self',null=True,blank=True,on_delete=True)
    # is_menus=models.BooleanField(default=False)
...
class Customer(models.Model):
    ...
    introduce_from = models.ForeignKey('self', verbose_name="转介绍自学员", blank=True, null=True,on_delete=True)  #self指的就是自己这个表，和下面写法是一样的效果


    # introduce_from = models.ForeignKey('Customer', verbose_name="转介绍自学员", blank=True, null=True,on_delete=models.CASCADE)
    course = MultiSelectField("咨询课程", choices=course_choices,null=True) #多选，并且存成一个列表的格式
    ...
    #用户表中存放的是自己公司的所有员工。
    consultant = models.ForeignKey('UserInfo', verbose_name="销售", blank=True, null=True,on_delete=True)


    #一个客户可以报多个班，报个脱产班，再报个周末班等，所以是多对多。
    class_list = models.ManyToManyField('ClassList', verbose_name="已报班级",) #去掉了null
...
class ConsultRecord(models.Model):
    ...
    跟进记录表
    """
    customer = models.ForeignKey('Customer', verbose_name="所咨询客户",on_delete=True)

    note = models.TextField(verbose_name="跟进内容...")

    status = models.CharField("跟进状态", max_length=8, choices=seek_status_choices, help_text="选择客户此时的状态")

    consultant = models.ForeignKey("UserInfo", verbose_name="跟进人", related_name='records',on_delete=True)
